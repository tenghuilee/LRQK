...
rand_methods = ["randn", "top", "topcol"]

base_model_llama = dict(
    type="LRQKChatBot",
...
base_model_qwen = dict(
    type="LRQKChatBot",
    abbr=f"Qwen2.5-7B-Instruct_LRQKChatBot_r32_act2048_lite64",
...
for rand_method in rand_methods:
    _llama = dict({
        k: v for k, v in base_model_llama.items()
    })
    _llama['abbr'] = _llama['abbr'] + f"_{rand_method}"
    _llama['lrqk_init_aq_ak_method'] = rand_method
    models.append(_llama)

    _qwen = dict({
        k: v for k, v in base_model_qwen.items()
    })
    _qwen['abbr'] = _qwen['abbr'] + f"_{rand_method}"
    _qwen['lrqk_init_aq_ak_method'] = rand_method
    models.append(_qwen)

del base_model_llama
del base_model_qwen
del rand_method
del _llama
del _qwen
